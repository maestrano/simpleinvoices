...
{foreach from=$invoiceItems item=invoiceItem }
		...
		{if $invoice.type_id == 2 }
			...
			<tr>
					...
					<td style="text-align:right">{$invoice.currency|htmlsafe}&nbsp;{$invoiceItem.unit_price|siLocal_number}</td>
					<td style="text-align:right">{$invoice.currency|htmlsafe}&nbsp;{$invoiceItem.gross_total|siLocal_number}</td>
	        </tr>
...
{/if}
	...
	<tr class="details_screen">
		...
		<td colspan="1" class="align_right">{if $invoice_number_of_taxes > 1}<u>{/if}{$invoice.currency|htmlsafe}&nbsp;{$invoice.gross|siLocal_number}{if $invoice_number_of_taxes > 1}</u>{/if}</td>
    </tr>
    {/if}
    ...
    {section name=line start=0 loop=$invoice.tax_grouped step=1}
    	...
    	<tr class="details_screen">
			...
			<td colspan="1" class="align_right">{$invoice.currency|htmlsafe}&nbsp;{$invoice.tax_grouped[line].tax_amount|siLocal_number}</td>
	    </tr>
	    {/if}
	...
	<tr class="details_screen">
		...
		<td colspan="1" class="align_right"><u>{$invoice.currency|htmlsafe}&nbsp;{$invoice.total_tax|siLocal_number}</u></td>
    </tr>
    {/if}
	{if $invoice_number_of_taxes > 1}
	<tr>
    ...
    <tr class="details_screen">
		...
		<td colspan="1" class="align_right"><span class="double_underline">{$invoice.currency|htmlsafe}&nbsp;{$invoice.total|siLocal_number}</span></td>
    </tr>
    {* tax section - end *}
</table>
...
<br /><br />
	...
	<tr>
		...
		<td class="account">{$invoice.currency|htmlsafe}&nbsp;{$invoice.total|siLocal_number}</td>
		<td class="account"><a href="index.php?module=payments&amp;view=manage&amp;id={$invoice.id|urlencode}">{$LANG.paid}:</a></td>
		<td class="account">{$invoice.currency|htmlsafe}&nbsp;{$invoice.paid|siLocal_number}</td>
		<td class="account">{$LANG.owing}:</td>
		<td class="account"><u>{$invoice.currency|htmlsafe}&nbsp;{$invoice.owing|siLocal_number}</u></td>
		...
		<td class="account">{$LANG.total}:</td>
		<td class="account">{$invoice.currency|htmlsafe}&nbsp;{$customerAccount.total|siLocal_number}</td>
		<td class="account"><a href="index.php?module=payments&amp;view=manage&amp;c_id={$customer.id|urlencode}">{$LANG.paid}:</a></td>
		<td class="account">{$invoice.currency|htmlsafe}&nbsp;{$customerAccount.paid|siLocal_number} </td>
		<td class="account">{$LANG.owing}:</td>
		<td class="account"><u>{$invoice.currency|htmlsafe}&nbsp;{$customerAccount.owing|siLocal_number}</u></td>
	</tr>
	</table>
<br />
